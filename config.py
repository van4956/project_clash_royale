...
def get_roi_bounds():
    ...
    try:
        if os.path.exists(ROI_CONFIG_PATH):
            ...
            logger.warning("Файл ROI не найден! (безопасные значения по умолчанию [0,0,1920,1080])")
            return (0, 0, 1920, 1080)
    except Exception as e:
        logger.error("Ошибка при чтении ROI: %s (безопасные значения по умолчанию [0,0,1920,1080])", e)
        return (0, 0, 1920, 1080)
