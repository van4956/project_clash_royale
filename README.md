...
![banner](data/banner_to_readme.jpg)
...
Основная механика:
1. Боевая колода составляется из 8 карт, из доступных >120 карт.
2. У каждой карты есть своя уникальная механика и стоимость в эликсире.
3. Разыгрывать можно по одной карте за раз, пока позволяет запас эликсира.
4. У каждого игрока свой запас эликсира.
5. Запас эликсир накапливается постепенно, автоматически, максимум до 10.
...
3. Каждая арена состоит из 2 боковых башен и 1 центральной.

Цель:
1. Уничтожить больше башен за отведенное время.
2. Уничтожение центральной башни = мгновенная победа.
...
На старте используем ОС Windows 11. В дальнейшем возможна миграция на Android систему.


## Общий план
- I. Обучение модели (бета-версия, модель на 20 классов)
- II. Написание кода (проверка потенциала, простая реализация)
- III. Масштабирование (обучение модели на 200 классов, улучшение кода)
...
2. Выделить область с игрой Clash Royale мышью
3. Нажмать **Enter** для подтверждения или **ESC** для отмены
4. Координаты сохраняются в файл `roi_config.txt`
